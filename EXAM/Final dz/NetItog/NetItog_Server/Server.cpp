...
void ProcessOrders()
{
	...
	while (true)
	{
		...
		this_thread::sleep_for(chrono::seconds(1)); // Ожидаем 1 секунду перед следующей проверкой
	}
}
...
int calculatePreparationTime(const string& originalOrder) // Функция для расчета времени приготовления заказа 
{
	...
	{
		auto found = preparationTime.find(item.first);
		...
		{
			totalTime += found->second * item.second; // Время приготовления * количество
			isValidOrder = true;
		}
	}
